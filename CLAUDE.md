...
- **Crafted-fixture pattern for header-dependent codes (DWI family done).** Codes that need a valid NIfTI header or specific malformed TSV content can't be reached by fetching valid example datasets. The durable way to close them: build a synthetic dataset in a Rust integration test ([crates/bids-core/tests/dwi_checks.rs](crates/bids-core/tests/dwi_checks.rs), shared builder in `tests/common/mod.rs`), confirm the code fires 1:1 vs Deno on the identical tree *during development*, then commit the assertion (CI-gated, no Deno/external data needed) and promote the code to `RUST_CODES`. Closed so far by this method: the **DWI** family (`VOLUME_COUNT_MISMATCH`, `BVAL_MULTIPLE_ROWS`, `BVEC_NUMBER_ROWS`, `DWI_MISSING_BVAL`, `DWI_MISSING_BVEC` — [dwi_checks.rs](crates/bids-core/tests/dwi_checks.rs)) and the **fmap** family (`FIELDMAP_WITHOUT_MAGNITUDE_FILE`, `MISSING_MAGNITUDE1_FILE`, `ECHOTIME1_2_DIFFERENCE_UNREASONABLE`, `MAGNITUDE_FILE_WITH_TOO_MANY_DIMENSIONS`, `EPI_WITH_BVALS_NEEDS_SMALL_BVALS`, `TOTAL_READOUT_TIME_MUST_DEFINE` — [fmap_checks.rs](crates/bids-core/tests/fmap_checks.rs)). Both were purely fixture-only — the generic engine, the `magnitude`/`bval` associations, and the NIfTI header context already implemented them. the **NIfTI-dimension** family (`T1W_FILE_WITH_TOO_MANY_DIMENSIONS`, `BOLD_NOT_4D`, `NIFTI_DIMENSION`, `NIFTI_PIXDIM` — [nifti_checks.rs](crates/bids-core/tests/nifti_checks.rs)), and the **ASL/perf** family — 25 codes — ([asl_checks.rs](crates/bids-core/tests/asl_checks.rs)): `aslcontext`/`m0scan`/nifti array-length checks, the `M0Type_*` trio, the `*_GREATER`/monotonic checks, the VolumeTiming mutual-exclusive cluster, `BACKGROUND_SUPPRESSION_PULSE_NUMBER_NOT_CONSISTENT`, `TOTAL_ACQUIRED_VOLUMES_NOT_CONSISTENT`. All fixture-only — even the feared `m0scan` association resolves at parity via the generic association arm. And the **PET** family (`PET_FRAME_CONSISTENCY`, `PET_FRAME_CONSISTENCY_FRAME_DURATION`, `PET_FRAME_CONSISTENCY_FRAME_TIMES_START`, `NIFTI_PIXDIM_PET` — [pet_checks.rs](crates/bids-core/tests/pet_checks.rs)). Next families by the same method: bold/mri timing (`REPETITION_TIME_GREATER_THAN`, `SLICETIMING_VALUES_GREATER_THAN_REPETITION_TIME`, `REPETITION_TIME_MISMATCH`, `EFFECTIVEECHOSPACING_*`), MRS (`MRS_*`), derivative metadata (`MISSING_RESOLUTION_DESCRIPTION`/`MISSING_DENSITY_DESCRIPTION` — need `DatasetType=="derivative"`), then standalone sidecar-value (`ECHO_TIME_GREATER_THAN`, `PHASE_UNITS`) and TSV-column checks (`EVENT_ONSET_ORDER`, `AGE_89`).
